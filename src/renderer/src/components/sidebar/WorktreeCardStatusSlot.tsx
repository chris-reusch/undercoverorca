...
const QUIET_BRANCH_REPLACEABLE_STATUSES = new Set<WorktreeStatus>(['active', 'done', 'inactive'])
// Why: a quiet workspace card surfaces its branch identity in the status lane
// instead of a redundant activity dot.
...
const branchStatusIconClassName = 'size-[13px] translate-x-px text-muted-foreground/70'
...
export function WorktreeCardStatusSlot({
  ...
  const canShowBranchStatus =
    newCardStyle && showStatus && hasBranchIdentity && QUIET_BRANCH_REPLACEABLE_STATUSES.has(status)
