...
export class StarNagService {
  ...
  // Why: dismissal backoff must use the prompt context that was delivered,
  // not whatever threshold/source happens to be current when the renderer
  // later reports a user action.
  ...
  private broadcastShow(
    ...
    this.promptSession = { source, mode }
  ...
  private defer(outcome: 'dismissed' | 'later'): void {
